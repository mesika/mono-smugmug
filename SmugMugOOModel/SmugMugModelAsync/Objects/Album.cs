    ...
    public class Album : SmugMugObject
    {
        ...
        public async Task GetCommentsAsync(bool Associative, int LastUpdated, string SitePassword)
        {
            ...
            if ((this != null) && !(string.IsNullOrEmpty(this.Password)))
                resp = await ch.ExecuteMethod<AlbumResponse>("smugmug.albums.comments.get", basic, "AlbumID", id, "AlbumKey", Key, "SitePassword", SitePassword, "Password", Password, "LastUpdated", LastUpdated, "Associative", Associative);
            else
                resp = await ch.ExecuteMethod<AlbumResponse>("smugmug.albums.comments.get", basic, "AlbumID", id, "AlbumKey", Key, "SitePassword", SitePassword, "LastUpdated", LastUpdated, "Associative", Associative);
            if (resp.stat == "ok")
            {
                //Hack: The class has 2 properties with the same name - Comments. I've separated them in 2 different properties (HasComments and CommentsList)
        ...
        {
            return new ImageUpload(basic.SessionID, this.id);
        }
    }
}
