...
namespace SmugMugModel
{
    public class Watermark : SmugMugObject
    {
        ...
        public bool Delete()
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.watermarks.delete", basic, "WatermarkID", id);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool ChangeSettings(string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.watermarks.changeSettings", basic, ls.ToArray());
            if (resp.stat == "ok")
                return true;
            else
            {
