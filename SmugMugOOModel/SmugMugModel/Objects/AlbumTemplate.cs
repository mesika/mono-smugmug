...
namespace SmugMugModel
{
    public class AlbumTemplate : SmugMugObject
    {
        ...
        internal AlbumTemplate Create()
        {
            ...
            var resp = ch.ExecuteMethod<AlbumTemplateResponse>("smugmug.albumtemplates.create", basic, ls.ToArray());
            if (resp.stat == "ok")
            {
        ...
        public bool ChangeSettings()
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.albumtemplates.changeSettings", basic, ls.ToArray());
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool Delete()
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.albumtemplates.delete", basic, "AlbumTemplateID", id);
            if (resp.stat == "ok")
                return true;
            else
            {
