...
namespace SmugMugModel
{
    public class Image : SmugMugObject
    {
        ...
        public bool ChangePosition(int Position)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.changePosition", basic, "ImageID", id, "Position", Position);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool ChangeSettings(string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.changeSettings", basic, ls.ToArray());
        ...
        public bool Crop(int Height, int Width, int X, int Y)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.crop", basic, "ImageID", id, "Height", Height, "Width", Width, "X", X, "Y", Y);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool ZoomThumbnail(int Height, int Width, int X, int Y)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.zoomThumbnail", basic, "ImageID", id, "Height", Height, "Width", Width, "X", X, "Y", Y);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public Image GetInfo(bool Sandboxed, string SitePassword)
        {
            ...
            ImageResponse resp = new ImageResponse();
            if ((Album != null) && (Album.Password != null) && (Album.Password != String.Empty))
                resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.getInfo", basic, "ImageID", id, "ImageKey", Key, "Sandboxed", Sandboxed, "SitePassword", SitePassword, "Password", Album.Password);
            else
                resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.getInfo", basic, "ImageID", id, "ImageKey", Key, "Sandboxed", Sandboxed, "SitePassword", SitePassword);
            if (resp.stat == "ok")
            {
                var temp = resp.Image;
                temp.basic = basic;
        ...
        {
            PopulateImageWithURLsFromSite(false, "", "");
        }
        ...
        public Image GetStats(int Month, int Year)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.getStats", basic, "ImageID", id, "Month", Month, "Year", Year);
            if (resp.stat == "ok")
            {
                var temp = resp.Image;
                temp.basic = basic;
        ...
        public bool GetPricing()
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.getPricing", basic, "ImageID", id);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool ApplyWatermark(Watermark w)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.applyWatermark", basic, "ImageID", id, "WatermarkID", w.id);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool RemoveWatermark()
        {
            CommunicationHelper ch = new CommunicationHelper();
            // SessionID [required], ImageId [required], Callback, Pretty, Strict
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.removeWatermark", basic, "ImageID", id);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public Comment AddComment(string Text, int Rating, string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<CommentResponse>("smugmug.images.comments.add", basic, "ImageID", id, "ImageKey", Key, "Text", Text, "Rating", Rating, "Extras", Extras);
            if (resp.stat == "ok")
            {
        ...
        public Printmark CreatePrintmark(string Name, int Dissolve, string Location, string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<PrintmarkResponse>("smugmug.printmarks.create", basic, "ImageID", this.id, "Name", Name, "Dissolve", Dissolve, "Location", Location, "Extras", Extras);
            if (resp.stat == "ok")
            {
        ...
        public Image CollectIntoAlbum(string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.collect", basic, "AlbumID", this.Album.id, "ImageID", this.id, "ImageKey", this.Key,"Extras", Extras);
            if (resp.stat == "ok")
            {
