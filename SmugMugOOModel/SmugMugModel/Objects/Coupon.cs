...
namespace SmugMugModel
{
    public class Coupon : SmugMugObject
    {
        ...
        public bool GetInfo()
        {
            ...
            var resp = ch.ExecuteMethod<CouponResponse>("smugmug.coupons.getInfo", basic, "CouponID", this.id);
            if (resp.stat == "ok")
            {
                if (resp.Coupon != null)
                {
        ...
        internal Coupon CreateCoupon(string AlbumIDs, string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<CouponResponse>("smugmug.coupons.create", basic, ls.ToArray());
            if (resp.stat == "ok")
            {
        ...
        public bool Modify(string AlbumIDs, string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<CouponResponse>("smugmug.coupons.modify", basic, "AlbumIDs", AlbumIDs, ls.ToArray());
            if (resp.stat == "ok")
            {
                var tempCoupon = resp.Coupon;
                if (tempCoupon != null)
        ...
        public bool RemoveRestriction(Album CouponAlbum, string Extras)
        {
            CommunicationHelper ch = new CommunicationHelper();
            // SessionID [required], CouponID [required], AlbumID [required], Callback, Extras, Pretty, Strict
            var resp = ch.ExecuteMethod<CouponResponse>("smugmug.coupons.restrictions.albums.remove", basic, "CouponID", this.id, "AlbumID", CouponAlbum.id, "Extras", Extras);
            if (resp.stat == "ok")
            {
                if (this.Restrictions != null)
                {
