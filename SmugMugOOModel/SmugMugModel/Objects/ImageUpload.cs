...
namespace SmugMugModel
{
    ...
    {
    	public float KBperSecond {get; internal set;}
    ...
    public class ImageUpload
    {
        ...
        public Image UploadImage(string filename, string MD5Checksum)
        {
            ...
            //int timeOut = ((int)myFileInfo.Length / 1024) * 10000;
            //myWebRequest.Timeout = timeOut;

			myWebRequest.KeepAlive = true;
			myWebRequest.Timeout = System.Threading.Timeout.Infinite;
			myWebRequest.ReadWriteTimeout = System.Threading.Timeout.Infinite;
			myWebRequest.AllowWriteStreamBuffering = false;
                ...
                using (FileStream sr = new FileStream(myFileInfo.FullName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    using (var stream = myWebRequest.GetRequestStream())
                    {
                        ...
                        while (howMuchRead < myFileInfo.Length)
                        {
							...
							stopWatchBytes +=readSoFar;
							
                            //we now write those files to the web.
                            stream.Write(buffer, 0, readSoFar);
                            //System.Convert.ToBase64String(buffer);

							float kBs = 1.0f * stopWatchBytes / stopWatch.ElapsedMilliseconds;

                            if (UploadProgress != null)
                                UploadProgress(this, new UploadEventArgs() { KBperSecond = kBs, FileName = myFileInfo.FullName, PercentComplete = (float)howMuchRead / (float)myFileInfo.Length });
                        }
                    }
                }
            }
