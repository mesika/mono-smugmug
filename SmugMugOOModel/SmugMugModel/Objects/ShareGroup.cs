...
namespace SmugMugModel
{
    public class ShareGroup : SmugMugObject
    {
        ...
        public bool Delete()
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.sharegroups.delete", basic, "ShareGroupID", id);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public ShareGroup GetInfo(string Password, string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<ShareGroupResponse>("smugmug.sharegroups.getInfo", basic, "ShareGroupID", id, "Password", Password, "ShareGroupTag", this.Tag, "Extras", Extras);
            if (resp.stat == "ok")
            {
        ...
        public bool Modify(string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.sharegroups.modify", basic, ls.ToArray());
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool Modify(bool AccessPassworded, string Description, string Name, string Password, string PasswordHint, string Extras)
        {
            CommunicationHelper ch = new CommunicationHelper();
            // SessionID [required], ShareGroupID [required], AccessPassworded, Callback, Description, Extras, Name, Password, PasswordHint, Pretty, Strict
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.sharegroups.modify", basic, "AccessPassworded", AccessPassworded, "Description", Description, "Extras", Extras, "Name", Name, "Password", Password, "PasswordHint", PasswordHint);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool AddAlbum(Album AlbumToAdd)
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.sharegroups.albums.add", basic, "ShareGroupID", this.id, "AlbumID", AlbumToAdd.id);
            if (resp.stat == "ok")
            {
                if (this.Albums == null)
        ...
        public bool RemoveAlbum(Album AlbumToRemove)
        {
            CommunicationHelper ch = new CommunicationHelper();
            // SessionID [required], ShareGroupID [required], AlbumID [required], Callback, Pretty, Strict 
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.sharegroups.albums.remove", basic, "ShareGroupID", id, "AlbumID", AlbumToRemove.id);
            if (resp.stat == "ok")
            {
                if (this.Albums != null)
                {
        ...
        public List<Album> GetAlbums(bool Associative, string Password, string Extras)
        {
            ...
            var resp = ch.ExecuteMethod<ShareGroupResponse>("smugmug.sharegroups.albums.get", basic, "ShareGroupTag", Tag, "Associative", Associative, "Password", Password, "Extras", Extras);
            if (resp.stat == "ok")
            {
