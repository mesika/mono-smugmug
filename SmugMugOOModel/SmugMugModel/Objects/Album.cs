...
namespace SmugMugModel
{
    ...
    {
		private Logger _logger = LogManager.GetLogger("Album");
        ...
        public bool Delete()
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.albums.delete", basic, "AlbumID", id);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public bool RemoveWatermark()
        {
            CommunicationHelper ch = new CommunicationHelper();
            // SessionID [required], AlbumID [required], Callback, Pretty, Strict
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.albums.removeWatermark", basic, "AlbumID", id);
            if (resp.stat == "ok")
                return true;
            else
            {
        ...
        public Image UploadImageFromURL(string URL)
        {
            ...
            var resp = ch.ExecuteMethod<ImageResponse>("smugmug.images.uploadFromURL", basic, "AlbumID", id, "URL", URL);
            if (resp.stat == "ok")
            {
                var temp = new Image();
                temp.basic = basic;
